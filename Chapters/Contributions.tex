...
\section{ReSA - a Constrained Requirements Specification Language of Embedded Systems}
...
%\subsection*{Embedded System Concepts}
%\begin{table}[h]\small
...
%	\end{tabular}
%\caption{Embedded system concepts.}\label{tbl_resa_concepts}
%\end{table}
...
%\begin{enumerate}[label=(\roman*)]\setlength\itemsep{1em}
%	\item Location \texttt{Start} - In the \texttt{Start} location the automaton waits for its release according to the order of execution given in the \textit{slist} and, for the discrete-time blocks, the offset parameter;
%	\item Location \texttt{Operate}-The \texttt{Operate} location models the operational mode of a Simulink block. For the discrete-time pattern, this location is decorated with an invariant connected to the block's period. For the continuous-time pattern, the location is decorated with an exponential rate $\lambda$ that determines the probability of the automaton to remain in this location at each simulation step, according to an exponential distribution, in other words Pr(leaving \texttt{Operate} after $t$)$ = 1-e^{-\lambda t}$. 
%	
%	\item Edge \texttt{(Start, Operate)} - The edge is enabled when the guard condition for releasing the block is satisfied. The time for release for continuous blocks (Figure~\ref{fig:continuouspattern}) is given as \texttt{gtime $\geq$ sn*IAT}, where: \texttt{gtime} is the global clock, \texttt{sn} is the block's execution number, and \texttt{IAT} is the constant inter-arrival time between two consecutive releases of automata. For the discrete blocks (Figure~\ref{fig:discretepattern}) the release time depends on the \texttt{offset} parameter that denotes the delay of the first execution of the block. When the edge is traversed, two update actions (modeled as C functions) are executed: \texttt{initialize()}, which initializes the data variables, and \texttt{blockRoutine()} that updates the output.
%	\item Edge \texttt{(Operate, Operate)} - The edge is taken every time the automaton updates its output. As explained above, the edge is traversed according to exponential distribution for continuous-time patterns, or at discrete-time intervals for the discrete-time patterns.  
%\end{enumerate}
...
%The Simulink model is transformed into the input language of UPPAAL SMC following according to formalization into a network of stochastic timed automata discussed in the previous sections. The requirements (or properties) need to be checked are specified in the query language of the model checker, that is in WMTL. Then, the UPPAAL SMC takes in the automata and the query, then after sufficient simulation of the automata, the model checker collects the simulation traces and applies statistical anlaysis, which results in a probabilistic verdict such as the probablity of satisfying the query and the confidence of the result.
...
\section{Scalable Software Allocation via Hybrid PSO}
